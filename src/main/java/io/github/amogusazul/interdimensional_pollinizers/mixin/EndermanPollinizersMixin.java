...
public abstract class EndermanPollinizersMixin extends MobEntityMixin {
	...
	void stub(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, EntityData entityData, NbtCompound entityNbt, CallbackInfoReturnable<EntityData> cir) {
		...
		final int seed = random.range(0, InterdimensionalPollinizersConfig.INSTANCE.seeds.value().size() - 1);

		if (random.range(1, 100) <= InterdimensionalPollinizersConfig.INSTANCE.pollinizers_percentage.value()) { //50% of spawning an item holding enderman
			this.setCarriedBlock(Registry.BLOCK.get(new Identifier(InterdimensionalPollinizersConfig.INSTANCE.seeds.value().get(seed))).getDefaultState());
		}
